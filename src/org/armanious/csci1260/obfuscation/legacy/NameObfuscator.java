...
import org.armanious.csci1260.obfuscation.ObfuscatedNameGenerator;
...
public class NameObfuscator {
	...
	public NameObfuscator(String name_pattern, int name_length,
			boolean preserve_package_structure) {
		this.preservePackageStructure = preserve_package_structure;
		ong = new ObfuscatedNameGenerator(name_pattern, name_length);

		packageNameRemapping = preservePackageStructure ? new HashMap<>() : null;
		classNameRemapping = new HashMap<>();
		fieldNameRemapping = new HashMap<>();
		methodNameRemapping = new HashMap<>();
	}

	private void populatePackageNameRemapping(ArrayList<ClassNode> classes){
		ong.reset();
	...
	private void populateClassNameRemapping(ArrayList<ClassNode> classes){
		ong.reset();
		@SuppressWarnings("unchecked")
		ArrayList<ClassNode> clone = (ArrayList<ClassNode>) classes.clone();
	...
	private void populateFieldNameRemapping(ArrayList<ClassNode> classes){
		for(ClassNode cn : classes){
			ong.reset();
			...
			while(superName != null){
				...
				for(ClassNode nextSuper : classes){
	...
	private void populateMethodNameRemapping(ArrayList<ClassNode> classes){
		for(ClassNode cn : classes){
			ong.reset();
			...
			otherOuter: for(String superName : toSearchSupers){
				outer: while(superName != null){
					for(ClassNode superCnPossibility : classes){
	...
	public void obfuscateNames(ArrayList<ClassNode> classes) {

		long start = System.currentTimeMillis();

		populatePackageNameRemapping(classes);
		populateClassNameRemapping(classes);
		populateFieldNameRemapping(classes);
		populateMethodNameRemapping(classes);

		long end = System.currentTimeMillis();

		System.out.println("It took " + (end - start) + " ms to map all obfuscated names.");
		
		start = System.currentTimeMillis();
		
		for(ClassNode cn : classes){ //tree walking format (implement visitor format?)
		...
		System.out.println("It took " + (end - start) + " ms to make the changes.");
	...
	public void outputObfuscationMapToFile(File outputFileForNameRemapping) {
		try(final BufferedWriter bw = new BufferedWriter(new FileWriter(outputFileForNameRemapping))){
			...
			System.err.println("Was not able to save obfuscation map to file " + outputFileForNameRemapping);
		}
	}
